...
class AgentRuntime:
    ...
    def _prepare(self, message: str, conversation_id: str | None) -> tuple[str, str, list[ModelMessage]]:
        clean = message.strip()
        if not clean:
        ...
        return clean, conversation_id, wire
    ...
    def _system_prompt(self, citations: list[RetrievedChunk]) -> str:
        if not citations:
    ...
    async def chat(self, message: str, conversation_id: str | None = None) -> ChatResponse:
        clean, conversation_id, wire = self._prepare(message, conversation_id)
        citations = await self._retrieve(clean)
        result: ModelResult = await self.model_gateway.complete(wire, self._system_prompt(citations))
    ...
    async def stream_chat(
        ...
        *,
        ...
        clean, conversation_id, wire = self._prepare(message, conversation_id)
        citations = await self._retrieve(clean)
        yield ChatStreamEvent(type="started", generation_id=generation_id, conversation_id=conversation_id, citations=citations)
        ...
        try:
            async for event in self.model_gateway.stream(wire, self._system_prompt(citations), cancel_event=cancel_event):
            ...
            yield ChatStreamEvent(
                ...
                fallback=fallback, finish_reason="cancelled", citations=citations,
            )
            return
            ...
            yield ChatStreamEvent(
                ...
                content=content or None, message_id=message_id, provider=provider, model=model,
                fallback=fallback, error=type(exc).__name__, finish_reason="error", citations=citations,
            )
            return
        ...
        if not content:
            yield ChatStreamEvent(
                type="error", generation_id=generation_id, conversation_id=conversation_id,
                provider=provider, model=model, fallback=fallback, error="empty_stream", finish_reason="error", citations=citations,
            )
            return
        ...
        yield ChatStreamEvent(
            ...
            fallback=fallback, error=model_error, finish_reason=finish_reason, citations=citations,
        )

...
public struct TaskEvent: Identifiable, Codable, Equatable, Sendable {
    ...
    /// Resulting task revision after this event was committed.
    public let revision: Int
...
public enum TaskValidation {
    ...
    public static func title(_ value: String) throws -> String {
        ...
        guard !normalized.isEmpty, normalized.count <= maximumTitleCharacters else {
    ...
    public static func instruction(_ value: String) throws -> String {
        let normalized = value.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalized.isEmpty, normalized.count <= maximumInstructionCharacters else {
            throw TaskStoreError.invalidInstruction
        }
        return normalized
    }
    ...
    public static func detail(_ value: String?) throws -> String? {
        ...
        let normalized = value.trimmingCharacters(in: .whitespacesAndNewlines)
        guard normalized.count <= maximumDetailCharacters else {
            throw TaskStoreError.detailTooLong
        }
    ...
    public static func maxAttempts(_ value: Int, alreadyAttempted: Int = 0) throws -> Int {
        guard
            (1...maximumAttempts).contains(value),
            value >= alreadyAttempted
        else {
            throw TaskStoreError.invalidMaxAttempts
        }
        return value
    }
}
...
public struct TaskStateMachine: Sendable {
    ...
    public func validateTransition(from record: TaskRecord, to newState: TaskState) throws {
        guard record.state != newState else {
            throw TaskStoreError.invalidTransition(from: record.state, to: newState)
        }
        ...
        case .running:
            allowed = [.waitingForPermission, .succeeded, .failed, .cancelled, .interrupted]
        case .waitingForPermission:
            allowed = [.running, .failed, .cancelled, .interrupted]
        case .interrupted:
            allowed = [.ready, .cancelled]
        case .failed:
            allowed = [.ready, .cancelled]
        ...
        if newState == .running, record.state == .ready, record.attemptCount >= record.maxAttempts {
    ...
    public func resultingAttemptCount(from record: TaskRecord, to newState: TaskState) -> Int {
        if record.state == .ready, newState == .running {
...
public enum TaskStoreError: Error, CustomStringConvertible, Sendable, Equatable {
    ...
    public var description: String {
        ...
        case .revisionConflict(let id, let expected, let actual):
            return "Task \(id.description) revision conflict: expected \(expected), actual \(actual.map(String.init) ?? "none")."
